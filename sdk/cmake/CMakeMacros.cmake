...
function(add_dependency_edges _source)
    if(GENERATE_DEPENDENCY_GRAPH)
...
function(add_cd_file)
    if((${ARGC} GREATER 5) AND (ARGV2 STREQUAL DESTINATION))
        set(_CD_FASTPATH 1)
        if(ARGV0 STREQUAL "TARGET")
            set(_CD_TARGET ${ARGV1})
        elseif(ARGV0 STREQUAL "FILE")
            set(_CD_FILE ${ARGV1})
        else()
            unset(_CD_FASTPATH)
        endif()
        set(_CD_DESTINATION ${ARGV3})
        if((${ARGC} EQUAL 6) AND (ARGV4 STREQUAL "FOR"))
            set(_CD_FOR ${ARGV5})
        elseif((${ARGC} EQUAL 7) AND (ARGV4 STREQUAL "NO_CAB") AND (ARGV5 STREQUAL "FOR"))
            set(_CD_NO_CAB 1)
            set(_CD_FOR ${ARGV6})
        else()
            unset(_CD_FASTPATH)
    ...
    if(${_CD_DESTINATION} STREQUAL root)
        set(_CD_DESTINATION "")
    endif()
